export const emner = [
  {
    id: 'nerve-og-hormonsystemet',
    tittel: 'Nerve- og hormonsystemet',
    ikon: '🧠',
    farge: '#c0392b',
    beskrivelse: '',
    parent: 'kropp-og-helse',
    skjulFraForside: true,
    kapitler: [],
  },
  {
    id: 'seksualitet',
    tittel: 'Seksualitet',
    ikon: '🌸',
    farge: '#a01261',
    beskrivelse: '',
    parent: 'kropp-og-helse',
    skjulFraForside: true,
    kapitler: [],
  },
  {
    id: 'immunsystemet',
    tittel: 'Immunsystemet',
    ikon: '🛡️',
    farge: '#f1c40f',
    tekstMørk: true,
    beskrivelse: '',
    parent: 'kropp-og-helse',
    skjulFraForside: true,
    kapitler: [],
  },
  {
    id: 'geologi',
    tittel: 'Geologi',
    ikon: '🌋',
    farge: '#0e7060',
    beskrivelse: '',
    parent: 'jorda',
    skjulFraForside: true,
    kapitler: [],
  },
  {
    id: 'okologi',
    tittel: 'Økologi',
    ikon: '🦋',
    farge: '#1b7a3e',
    beskrivelse: '',
    parent: 'jorda',
    skjulFraForside: true,
    kapitler: [],
  },
  {
    id: 'celler-og-livets-utvikling',
    tittel: 'Celler og livets utvikling',
    ikon: '🦠',
    farge: '#8db600',
    tekstMørk: true,
    beskrivelse: '',
    parent: 'jorda',
    skjulFraForside: true,
    kapitler: [],
  },
  {
    id: 'teknologi',
    tittel: 'Teknologi',
    ikon: '⚙️',
    farge: '#1f6fa0',
    beskrivelse: '',
    parent: 'teknologi-og-vitenskap',
    skjulFraForside: true,
    kapitler: [],
  },
  {
    id: 'vitenskap',
    tittel: 'Vitenskap',
    ikon: '💡',
    farge: '#6d4aa7',
    beskrivelse: '',
    parent: 'teknologi-og-vitenskap',
    skjulFraForside: true,
    kapitler: [],
  },
  {
    id: 'jorda',
    tittel: 'Jorda',
    ikon: '🌍',
    farge: '#1b7a3e',
    beskrivelse: 'Geologi, økologi, celler og livets utvikling.',
    kapitler: [
      'Jordas indre og plater',
      'Bergarter og mineraler',
      'Vann og atmosfære',
      'Jorda i solsystemet',
    ],
  },
  {
    id: 'kropp-og-helse',
    tittel: 'Kropp og helse',
    ikon: '🧬',
    farge: '#c0392b',
    beskrivelse: 'Nerve- og hormonsystemet, immunsystemet og seksualitet.',
    kapitler: [
      'Celler og arvestoff',
      'Organsystemer',
      'Pubertet og seksualitet',
      'Sykdom og immunforsvar',
    ],
  },
  {
    id: 'teknologi-og-vitenskap',
    tittel: 'Teknologi og vitenskap',
    ikon: '⚙️',
    farge: '#1f6fa0',
    beskrivelse: 'Vitenskap, programmering og teknologiske løsninger og utfordringer.',
    kapitler: ['Teknologi', 'Vitenskap'],
  },
  {
    id: 'kjemi',
    tittel: 'Kjemi',
    ikon: '🧪',
    ikonSrc: 'icons/erlenmeyer.svg',
    farge: '#8e44ad',
    beskrivelse: 'Stoffenes oppbygning, kjemiske reaksjoner og periodesystemet.',
    kapitler: [
      'Atomet og periodesystemet',
      'Kjemiske bindinger',
      'Kjemiske reaksjoner',
      'Syrer og baser',
    ],
  },
  {
    id: 'klima-og-baerekraft',
    tittel: 'Klima og bærekraft',
    ikon: '🌱',
    farge: '#0e7a8a',
    beskrivelse: 'Klimaendringer, naturressurser og bærekraftig utvikling.',
    kapitler: [
      'Klima og klimaendringer',
      'Naturressurser og forbruk',
      'Forurensning og avfall',
      'FNs bærekraftsmål',
    ],
  },
  {
    id: 'energi',
    tittel: 'Energi',
    ikon: '⚡',
    farge: '#f39c12',
    tekstMørk: true,
    beskrivelse: 'Energiformer, energioverføring og energiproduksjon.',
    kapitler: [
      'Energiformer og energikjeder',
      'Fornybar og ikke-fornybar energi',
      'Elektrisitet og strøm',
      'Energi, varme og bevegelse',
    ],
  },
];
